AJS.Confluence.EditorLoader = (function() {
    ...
    var getResources = function() {
        ...
        var resourceTags = AJS.Meta.get("quick.comment.resources");
    ...
    var loadEditor = function(successCallback, errorCallback) {
        ...
        $preloadContainer.load(Confluence.getContextPath() + "/plugins/quick-comment/editor.action",
                { 
...
AJS.$(function() {
    ...
    var reloadPageSaveCommentHandler = function(data) {
        var currentUrl = AJS.Meta.get("canonical-url");
        ...
        var reloadUrl = currentUrl + "focusedCommentId=" + data.id + "#comment-" + data.id;
        window.location.href = reloadUrl;
    };
    
    var confirmCancelHandler = function() {
        AJS.Rte.Content.editorResetContentChanged();
    ...
    var QuickComment = {
        ...
        createSaveHandler: function(successHandler, errorHandler) {
            ...
            return function(event) {
                ...
                var $captchaInput = AJS.$('input[name="captchaResponse"]', $form);
                var captcha = null;
                ...
                var changeCaptchaErrorHandler = function(message) {
                    ...
                    if ($img.length) {
                        ...
                        $img[0].src = src + "?" + (new Date()).getTime(); // break the page-memory caching done by browser (which ignores cache headers).
                        $captchaInput.val("");
                    }
        ...
        /**
         * Create a function to be activated when the editor is cancelled. This is a function taking a single event parameter.
         * 
         * @param confirmHandler a zero argument function to be called if the user confirms the discard.
         * @param abortHandler the zero argument function to be called if the user cancels the discard.
         * @return a cancel handler.
         */
        createCancelHandler: function(confirmHandler, abortHandler) {
            return function() {
                Confluence.Editor.checkAndConfirmDiscard(confirmHandler, abortHandler);     
            };
        },
        ...
        saveCommentErrorHandler: function(message) {
            ...
            AJS.Confluence.EditorNotification.notify("error", "Failed to save the comment:" + " " + message, 30);
        }
    };
...
//            };
            ...
            var cancelHandler = QuickComment.createCancelHandler(confirmCancelHandler, null);
...
//            };
        ...
        loadingTransition: function($form, $loadingContainer, $spacer) {
            ...
            if (!QuickComment.Reply.scrollWindowToElement($container)) {
                // try and scroll the closest overflow set parent (e.g. Documentation Theme)
                QuickComment.Reply.scrollOverflowContainerToElement($container);
            }
         ...
         * to move the window.
         */
        scrollWindowToElement: function($element) {
            function getScrollY() {
         ...
         */
        scrollOverflowContainerToElement: function($element) {
            var $parent = null;
...
Confluence.Templates.Comments.userLogo = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  output.append('<p class="comment-user-logo">', (opt_data.userInfo.userName == null) ? '<img class="userLogo logo anonymous" src="' + soy.$$escapeHtml(opt_data.context.staticResourceUrlPrefix) + '/images/icons/profilepics/anonymous.png" alt="' + soy.$$escapeHtml("User icon") + ': ' + soy.$$escapeHtml("Anonymous") + '" title="' + soy.$$escapeHtml("Anonymous") + '">' : (opt_data.userInfo.profilePicture.isDefault) ? '<a class="userLogoLink" data-username="' + soy.$$escapeHtml(opt_data.userInfo.userName) + '" href="' + soy.$$escapeHtml(opt_data.context.contextPath) + '/users/editmyprofilepicture.action" title="' + soy.$$escapeHtml("\u6dfb\u52a0\u5934\u50cf") + '"><img class="userLogo logo defaultLogo" src="' + soy.$$escapeHtml(opt_data.context.staticResourceUrlPrefix) + '/images/icons/profilepics/add_profile_pic.png" alt="' + soy.$$escapeHtml("User icon") + ': ' + soy.$$escapeHtml("\u6dfb\u52a0\u5934\u50cf") + '"></a>' : '<a class="userLogoLink" data-username="' + soy.$$escapeHtml(opt_data.userInfo.userName) + '" href="' + soy.$$escapeHtml(opt_data.context.contextPath) + '/display/~' + soy.$$escapeUri(opt_data.userInfo.userName) + '"><img class="userLogo logo" src="' + soy.$$escapeHtml(opt_data.context.contextPath) + soy.$$escapeHtml(opt_data.userInfo.profilePicture.path) + '" alt="' + soy.$$escapeHtml("User icon") + ': ' + soy.$$escapeHtml(opt_data.userInfo.userName) + '" title="' + soy.$$escapeHtml(opt_data.userInfo.userName) + '"></a>', '</p>');
  return opt_sb ? '' : output.toString();
};
...
Confluence.Templates.Comments.displayComment = function(opt_data, opt_sb) {
  ...
  if (opt_data.comment.parentId == 0 && opt_data.firstReply) {
    output.append('<div id="comments-section" class="pageSection group"><div class="section-header"><h2 id="comments-section-title" class="section-title"><a href="#" class="comments-show-hide comments-showing" name="comments">', soy.$$escapeHtml("\u8bc4\u8bba"), '</a></h2><a title="', soy.$$escapeHtml("Hide/Show Comments"), '" href="#" class="comments-show-hide icon comments-showing icon-section-opened">', soy.$$escapeHtml("Hide/Show Comments"), '</a>');
...
Confluence.Templates.Comments.commentView = function(opt_data, opt_sb) {
  ...
  Confluence.Templates.Comments.userLogo({userInfo: opt_data.commenter, context: opt_data.context}, output);
  output.append('<p class="date" title="', soy.$$escapeHtml("\u5927\u7ea61\u5206\u949f\u4ee5\u524d"), '">', soy.$$escapeHtml("\u5927\u7ea61\u5206\u949f\u4ee5\u524d"), '</p><div class="comment-header"><h4 class="author">', (opt_data.commenter.userName == null) ? soy.$$escapeHtml("\u533f\u540d\u7528\u6237") : '<a href="' + soy.$$escapeHtml(opt_data.context.contextPath) + '/display/' + soy.$$escapeUri(opt_data.commenter.userName) + '" class="url fn confluence-userlink" data-username="' + soy.$$escapeUri(opt_data.commenter.userName) + '">' + soy.$$escapeHtml(opt_data.commenter.displayName) + '</a>', '</h4></div><div class="comment-body"><div class="comment-content wiki-content">', opt_data.comment.html, '</div><div class="comment-actions">');
...
Confluence.Templates.Comments.displayCommentActions = function(opt_data, opt_sb) {
  ...
  if (opt_data.actions != null) {
    var itemList224 = opt_data.actions;
    ...
    for (var itemIndex224 = 0; itemIndex224 < itemListLen224; itemIndex224++) {
      ...
      output.append('<li ', (itemData224.style != null) ? ' class="' + soy.$$escapeHtml(itemData224.style) + '"' : '', '><a ', (itemData224.tooltip != null) ? ' title="' + soy.$$escapeHtml(itemData224.tooltip) + '"' : '', ' href="', soy.$$escapeHtml(itemData224.url), '" ', (itemData224.id) ? ' id="' + soy.$$escapeHtml(itemData224.id) + '-' + soy.$$escapeHtml(opt_data.commentId) + '"' : '', '><span>', soy.$$escapeHtml(itemData224.label), '</span></a></li>\n');
    }
  }
  output.append('</ul>');
  return opt_sb ? '' : output.toString();
...
Confluence.CommentDisplayManager = (function($) {
    ...
    return {
         ...
         */
        addComment: function(commenter, comment, highlight) {
            ...
            if (!this.hasComments()) {
                ...
                if (comment.parentId == 0) {
                    ...
                    var $commentThread = AJS.$("#comment-thread-" + commenter.parentId);
            ...
            $insertedComment[0].scrollIntoView();
        },        
...
AJS.bind("init.rte", function() {
    ...
    Confluence.Editor.CommentManager = (function($) {
        ...
        function getAddNewCommentUrl(contentId, actions) {
            var url = Confluence.getContextPath() + "/rest/editor/1.0/content/" + contentId + "/comment";
            if (actions) {
                url += "?actions=true";
            }
        ...
        function getReplyToCommentUrl(contentId, parentCommentId, actions) {
            var url = Confluence.getContextPath() + "/rest/editor/1.0/content/" + contentId + "/comments/" + parentCommentId + "/comment";
            if (actions) {
                url += "?actions=true";
            }
        ...
        return {
            ...
            saveComment: function(contentId, parentId, editorHtml, watch, captcha, successCallback, errorCallback) {
                ...
                if (captcha) {
                    ajaxData.headers["X-Atlassian-Captcha"] = captcha;
                }
