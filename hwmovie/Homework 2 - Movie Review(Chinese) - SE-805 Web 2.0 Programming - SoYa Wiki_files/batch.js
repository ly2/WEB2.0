...
Confluence.Templates.CreateSpace.createSpaceForm = function(opt_data, opt_sb) {
  ...
  output.append('</div><div class="field-group"><label for="key">', soy.$$escapeHtml("Space key"), '<span class="aui-icon icon-required"></span></label><input class="medium-field" name="key" ', (opt_data.key) ? 'value="' + soy.$$escapeHtml(opt_data.key) + '"' : '', '/><a class="icon icon-info" href="#" id="space-key-info-link">', soy.$$escapeHtml("\u5e2e\u52a9"), '</a>');
  ...
  output.append('<input type="hidden" name="atl_token" value="', soy.$$escapeHtml(opt_data.atlToken), '" />', (opt_data.buttons) ? '<div class="buttons-container"><div class="buttons"><input name="create" class="button" type="submit" value="' + soy.$$escapeHtml("\u6dfb\u52a0") + '" /><a href="' + soy.$$escapeHtml("/wiki") + '/dashboard.action">' + soy.$$escapeHtml("\u53d6\u6d88") + '</a></div></div>' : '', '</form>');
  return opt_sb ? '' : output.toString();
};
...
Confluence.Templates.CreateSpace.createPersonalSpaceForm = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  output.append('<form action="', soy.$$escapeHtml("/wiki"), '/spaces/docreatepersonalspace.action" method="post" class="aui" id="create-personal-space-form"><p>', "\x3cp\x3e\x3cb\x3e\u4e2a\u4eba\u7a7a\u95f4\x3c/b\x3e\u662f\u60a8\u5728Confluence\u4e2d\u7684\u72ec\u7acb\u7a7a\u95f4\u3002\x3c/p\x3e\x3cp\x3e\u60a8\u53ef\u4ee5\u5141\u8bb8\u8ba9\u5176\u4ed6\u4eba\u5728\u60a8\u7684\u4e2a\u4eba\u7a7a\u95f4\u4e2d\x3cb\x3e\u67e5\u770b\x3c/b\x3e\u6216\x3cb\x3e\u521b\u5efa\x3c/b\x3e\u5185\u5bb9\u3002\x3c/p\x3e", '</p>');
  Confluence.Templates.CreateSpace.spaceFormPermissions({confluenceFlavour: opt_data.confluenceFlavour, spacePermission: (opt_data.spacePermission) ? soy.$$escapeHtml(opt_data.spacePermission) : 'edit'}, output);
  output.append('<input type="hidden" name="atl_token" value="', soy.$$escapeHtml(opt_data.atlToken), '" />', (opt_data.buttons) ? '<div class="buttons-container"><div class="buttons"><input name="create" class="button" type="submit" value="' + soy.$$escapeHtml("\u6dfb\u52a0") + '" /><a href="' + soy.$$escapeHtml("/wiki") + '/dashboard.action">' + soy.$$escapeHtml("\u53d6\u6d88") + '</a></div></div>' : '', '</form>');
  return opt_sb ? '' : output.toString();
};
...
(function(e){var d=function(g){var f=g.siblings(".error");e.getJSON(AJS.Confluence.getContextPath()+"/ajax/spaceavailable.action",{key:g.val()},function(h){f.remove();if(!h.available&&h.message){g.parent().append('<div id="'+g.attr("name")+'-error" class="error">'+h.message+"</div>")}})},a=function(f){var g=f.find("input[name='key']").first();g.generateFrom(f.find("input[name='name']").first(),{uppercase:false,validationCallback:function(){d(g)}});g.change(function(){d(g)});new AJS.Tooltip({anchor:f.find("#space-key-info-link"),text:"Usually a unique, short version of a space&#8217;s name. It is used in links to content in this space.",width:150})},b=function(){var g=e(Confluence.Templates.CreateSpace.createSpaceForm({atlToken:e("#atlassian-token").prop("content"),confluenceFlavour:AJS.Meta.get("confluence-flavour")})),f=function(){g.submit()},h=new AJS.ConfluenceDialog({width:540,height:370,id:"create-space-dialog",closeOnOutsideClick:true,onSubmit:f});a(g);h.addHeader("Add a space");h.addPanel("Panel 1",g);h.addSubmit("\u6DFB\u52A0",f);h.addCancel("\u53D6\u6D88",function(){h.hide()});h.gotoPage(0);h.gotoPanel(0);return h},c=null;Confluence.CreateSpace={showDialog:function(){if(c!==null){c.show();return}c=b();c.show()},bindToForm:a}})(AJS.$);
AJS.toInit(function(d){var c=null,b=function(){var f=d(Confluence.Templates.CreateSpace.createPersonalSpaceForm({atlToken:d("#atlassian-token").prop("content"),confluenceFlavour:AJS.Meta.get("confluence-flavour")})),e=function(){f.submit()},g=new AJS.ConfluenceDialog({width:540,height:300,id:"create-personal-space-dialog",closeOnOutsideClick:true,onSubmit:e});g.addHeader("Add your personal space");g.addPanel("Panel 1",f);g.addSubmit("\u6DFB\u52A0",e);g.addCancel("\u53D6\u6D88",function(){g.hide()});g.gotoPage(0);g.gotoPanel(0);return g},a=function(){if(c==null){c=b()}c.show();d("#create-personal-space-dialog button.button-panel-submit-button").focus()};d("#create-personal-space-link").click(function(){a();return false})});
